...
void metodoPLU(MatrixLIL* A_lil, double* b, double* x, int* iter, double* tiempo) {
    ...
    // Estructuras CSR Dinámicas para L y U
    ...
    // ACUMULADOR DENSO (SPA - Scatter/Gather)
    double* w = (double*)calloc(n, sizeof(double));

    for (int i = 0; i < n; i++) {
        // SCATTER: Esparcir la fila de A_lil en Caché
        for (int k = 0; k < A_lil->filas[i].tamano; k++) {
            w[A_lil->filas[i].datos[k].col] = A_lil->filas[i].datos[k].val;
        }

        // OPERAR (Doolittle O(1) usando SPA)
        for (int j = 0; j < i; j++) {
            if (fabs(w[j]) > 1e-15) {
                w[j] *= U->inv_diag[j]; // División por U_{jj}
                // Actualizar resto de la fila
        ...
        // GATHER: Guardar en estructuras sin memmove
        L->inv_diag[i] = 1.0; // La diagonal de L siempre es 1 implícito
        for (int j = 0; j < n; j++) {
            if (fabs(w[j]) > 1e-15 || i == j) {
                if (j < i) { // Anexar a L
                    if (nnz_L >= cap_L) { cap_L *= 2; L->valores = (double*)realloc(L->valores, cap_L*sizeof(double)); L->columnas = (int*)realloc(L->columnas, cap_L*sizeof(int)); }
                    L->valores[nnz_L] = w[j]; L->columnas[nnz_L] = j; nnz_L++;
                } else { // Anexar a U
                    if (nnz_U >= cap_U) { cap_U *= 2; U->valores = (double*)realloc(U->valores, cap_U*sizeof(double)); U->columnas = (int*)realloc(U->columnas, cap_U*sizeof(int)); }
                    if (j > i) {
            ...
            w[j] = 0.0; // Resetear el acumulador
        }
    ...
    // RESOLVER (Sustituciones ultra veloces CSR)
...
void metodoCholesky(MatrixLIL* A_lil, double* b, double* x, int* iter, double* tiempo) {
    ...
    double* w = (double*)calloc(n, sizeof(double)); // ACUMULADOR (SPA)

    for (int i = 0; i < n; i++) {
        // SCATTER (Solo cargar parte triangular inferior)
        for (int k = 0; k < A_lil->filas[i].tamano; k++) {
            int col = A_lil->filas[i].datos[k].col;
            if (col <= i) w[col] = A_lil->filas[i].datos[k].val;
        }

        // OPERAR (Cholesky O(1) en SPA)
        for (int j = 0; j <= i; j++) {
            double suma = 0.0;
            // Unrolling sobre SPA
        ...
        // GATHER
        for (int j = 0; j < i; j++) { // Estrictamente menor para L
            if (fabs(w[j]) > 1e-15) {
                if (nnz_L >= cap_L) {
                    cap_L *= 2;
    ...
    // L^T * x = y usando columnas para evitar trasponer y gastar memoria
...
void metodoGradienteConjugado(MatrixLIL* A_lil, double* b, double* x, int* iter, double* tiempo) {
    ...
    // Conversión "Flash" a CSR y Precondicionador
    ...
    for (k = 0; k < n; k++) {
        ...
        if (rsnew < 1e-20) break; // Bypass de sqrt()
